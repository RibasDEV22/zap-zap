...
function validateCredentials(username, password) {
    ...
    if (!USERNAME_REGEX.test(cleanUser)) {
        throw new AuthError('Usuario deve ter entre 3 e 20 caracteres (apenas letras minusculas, numeros e _).');
    }
    if (!password || password.length < 4) {
        throw new AuthError('A senha deve ter no minimo 4 caracteres.');
    }
    return cleanUser;
}
...
async function registerUser(username, password, displayName, avatar) {
    ...
    if (existing) throw new AuthError('Este nome de usuario ja esta em uso.');
    ...
    return { username: cleanUser, displayName: finalDisplayName, avatar: avatar || '', role: role, bio: '' };
}

async function authenticateUser(username, password) {
    const cleanUser = (username || '').toLowerCase().trim();
    if (!cleanUser || !password) throw new AuthError('Informe usuario e senha.');
    const user = stmtGetUser.get(cleanUser);
    if (!user) throw new AuthError('Usuario ou senha incorretos.');
    const isValid = await bcrypt.compare(password, user.password);
    if (!isValid) throw new AuthError('Usuario ou senha incorretos.');
    return {
